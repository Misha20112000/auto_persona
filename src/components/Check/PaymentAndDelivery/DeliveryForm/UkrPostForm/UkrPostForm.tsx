...
export const UkrPostForm:React.FC<IPropsTypes> = ({name, phoneNumber, email, department,changeName,changePhoneNumber,changeEmail,changeDepartment}) => {
    return (
        <>
            <PersonalDataInput changeValue={changeName} value={name} title='Як до вас звертатись?' placeholder="Введіть своє ім'я"/>
            <PersonalDataInput type='number' changeValue={changePhoneNumber} value={phoneNumber} title='Номер телефону?' placeholder="Введіть номер терефону"/>
